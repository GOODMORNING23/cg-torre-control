...
export const useOtorgamiento = (fecha, cliente) => {
  const [datosOtorgamiento, setDatosOtorgamiento] = useState([]);
    ...
    const AtencionDatosFISA = async () => {
      ...
      try {
        const { data } = await DatosAxios(
          `/otorgamiento?cliente=${cliente}&fecha=${fecha}`,
          config
        );
        ...
        setDatosOtorgamiento(datosArray);
      } catch (error) {
        setDatosOtorgamiento([]);
      }
    };
    AtencionDatosFISA();
  }, [fecha, cliente]);
